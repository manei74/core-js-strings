 ...
 *
 * @bhparam {string} value - The input string to remove trailing whitespaces from.
...
function formatTime(minutes, seconds) {
  const newMin = minutes.toString();
  const newSec = seconds.toString();
  const final = `${newMin.padStart(2, '0')}:${newSec.padStart(2, '0')}`;
  return final;
}
...
function orderAlphabetically(str) {
  const newArr = [];
  let newStr = '';
  for (let i = 0; i < str.length; i += 1) newArr.push(str[i]);
  newArr.sort();
  for (let y = 0; y < newArr.length; y += 1) newStr += newArr[y];
  return newStr;
}
...
function isPalindrome(str) {
  const strComp = str.toLowerCase().replace(/[^a-z0-9]/g, '');
  let newStr = '';
  for (let i = strComp.length - 1; i >= 0; i -= 1) newStr += strComp[i];
  if (strComp === newStr) return true;
  return false;
}
 ...
 * Seegh details:  https://en.wikipedia.org/wiki/ROT13
 *
 * @param {string} str - The input string.
...
function encodeToRot13(str) {
  ...
  for (let i = 0; i < str.length; i += 1)
    if (/[a-zA-Z]/.test(str[i]))
      newStr += rot13Alphabet[realAlphabet.indexOf(str[i])];
    else newStr += str[i];
  return newStr;
}
